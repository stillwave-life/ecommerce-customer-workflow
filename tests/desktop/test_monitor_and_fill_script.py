...
SCRIPT = ROOT_DIR / "scripts" / "jd_customer_service_monitor_and_fill.py"


def test_jd_customer_service_monitor_and_fill_returns_idle_without_new_message():
    payload = {
        "probe_payload": {
            "window": {
                "handle": 1001,
    ...
    assert response.get("fill_result") is None


def test_jd_customer_service_monitor_and_fill_returns_fill_result_for_new_message():
    payload = {
        "probe_payload": {
            "window": {
                "handle": 1001,
    ...
    assert response["fill_result"]["reason"] in {"fill_provider_unavailable", "fill_execution_failed"}
